        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="bg-blue-100 min-h-screen flex items-center">
                        ...
                        <div class="w-full">
                            <h2 class="text-center text-blue-400 font-bold text-2xl uppercase mb-10">Edit Order</h2>
                            ...
                            <form action="{{ route('orders.update',$order->id)}}" method="POST" enctype='multipart/form-data'>
                                ...
                                <input type="text" id="name" name="name" placeholder="Customer Name" class="border border-gray-300 shadow p-3 w-full rounded mb-" value="{{$order->customer_name}}">
                                @error('name')
                                ...
                                <input type="text" id="name" name="mobile" placeholder="Mobile" class="border border-gray-300 shadow p-3 w-full rounded mb-" value="{{$order->mobile}}">
                                @error('mobile')
                                ...
                                <label for="twitter" class="block mb-2 font-bold text-gray-600">Product</label>
                                    ...
                                    <option value="{{$product->id}}" {{ $order->product_id == $product->id ? 'selected="selected"' : '' }}>{{$product->name}}</option>
                                    @endforeach
                                    </select>
                                ...
                                <input type="text" id="name" name="quantity" placeholder="Quantity" class="border border-gray-300 shadow p-3 w-full rounded mb-" value="{{$order->quantity}}">
                                @error('quantity')
